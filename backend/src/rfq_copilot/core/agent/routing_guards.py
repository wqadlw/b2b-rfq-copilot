...
SELECTION_CONSULT_MARKERS: tuple[str, ...] = (
    ...
    "优缺点",
    "选哪个",
    # 2026-09-23 实测补词："极限真空和抽速哪个更重要？"含双规格词被判 product_flow，
    # 0 命中后吐产品搜索兜底——概念对比问法与"怎么选"同域，属 ADR-0008 D1 护栏实现缺口。
    "哪个更重要",
    "更重要",
    "哪个关键",
    "哪个优先",
...
# P1-5 规格确定性路由守卫词：命中任一则交还 LLM（不 0-token 进 spec_match_flow）。
# 两类风险面——概念/比较问句（应走 LLM 选型咨询或知识通道）与商务问句
# （应走 refusal 拒绝或 LLM 精判）。eval 全族（A/B/C/D）在该词表下零触发（2026-09-21 实测）。
SPEC_ROUTE_GUARDS = (
    # 概念 / 比较 / 问句
    "意思",
    "是什么",
