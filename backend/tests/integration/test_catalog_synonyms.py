...
def _data_dir(tmp_path: Path, names, meta: dict | None) -> Path:
    ...
    (tmp_path / "knowledge.json").write_text(json.dumps(_knowledge(*names), ensure_ascii=False), encoding="utf-8")
    if meta is not None:
        (tmp_path / "search_meta.json").write_text(json.dumps(meta, ensure_ascii=False), encoding="utf-8")
    return tmp_path
...
async def test_synonym_expansion_widens_recall(tmp_path: Path) -> None:
    ...
    """
    names = ["水环真空泵"]
    without = OfflineProductCatalog(str(_data_dir(tmp_path / "a", names, None)))
    result = await without.search(ProductSearchQuery(keyword="液环泵"))
    assert result.total == 0  # 无元数据 → 无扩词 → 词集不相交，搜不到

    with_meta = OfflineProductCatalog(str(_data_dir(tmp_path / "b", names, {"synonyms": {"液环泵": "水环泵"}})))
...
async def test_whole_query_synonym_replacement(tmp_path: Path) -> None:
    ...
    cat = OfflineProductCatalog(str(_data_dir(tmp_path, ["无油真空泵"], {"synonyms": {"无油泵": "无油真空泵"}})))
...
async def test_block_words_return_empty_result(tmp_path: Path) -> None:
    cat = OfflineProductCatalog(str(_data_dir(tmp_path, ["真空泵"], {"block_words": ["赌博"]})))
...
async def test_search_meta_property_exposed(tmp_path: Path) -> None:
    cat = OfflineProductCatalog(str(_data_dir(tmp_path, ["真空泵"], {"synonyms": {"a": "b"}, "block_words": ["c"]})))
